...
TO_EMAIL = os.getenv("TO_EMAILS").split(",")  # RECEIVER's MAIL


def send_fraud_email(row):
    msg = EmailMessage()
    msg["Subject"] = f"🚨 Fraud Alert - Txn #{row['txn_id']} | Step {row['step']}"
    msg["From"] = GMAIL_USER
    msg["To"] = ", ".join(TO_EMAIL)

    # Plain text fallback
    msg.set_content(f"""
⚠️ A fraudulent transaction has been detected!
...
Amount: ₹{row['amount']}
Sender: {row['nameOrig']}
Old Balance (Sender): ₹{row['oldbalanceOrg']}
New Balance (Sender): ₹{row['newbalanceOrig']}
Receiver: {row['nameDest']}
Old Balance (Receiver): ₹{row['oldbalanceDest']}
New Balance (Receiver): ₹{row['newbalanceDest']}
...
""")
    ...
    <html>
      <body style="font-family: 'Segoe UI', sans-serif; background-color: #f4f6f8; padding: 20px;">
        <div style="max-width: 600px; margin: auto; background-color: #fff; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); padding: 20px; border-left: 5px solid #c0392b;">
          <h2 style="color: #c0392b;">🚨 Fraudulent Transaction Detected</h2>
          <p style="font-size: 15px;">The following transaction was flagged as potentially fraudulent:</p>
          <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
            <tr><td style="padding: 8px;"><strong>Transaction ID</strong></td><td>{row['txn_id']}</td></tr>
            ...
            <tr><td style="padding: 8px;"><strong>Amount</strong></td><td>₹{row['amount']}</td></tr>
            <tr><td style="padding: 8px;"><strong>Sender</strong></td><td>{row['nameOrig']}</td></tr>
            <tr><td style="padding: 8px;"><strong>Old Balance (Sender)</strong></td><td>₹{row['oldbalanceOrg']}</td></tr>
            <tr><td style="padding: 8px;"><strong>New Balance (Sender)</strong></td><td>₹{row['newbalanceOrig']}</td></tr>
            <tr><td style="padding: 8px;"><strong>Receiver</strong></td><td>{row['nameDest']}</td></tr>
            <tr><td style="padding: 8px;"><strong>Old Balance (Receiver)</strong></td><td>₹{row['oldbalanceDest']}</td></tr>
            <tr><td style="padding: 8px;"><strong>New Balance (Receiver)</strong></td><td>₹{row['newbalanceDest']}</td></tr>
          </table>
    ...
    try:
        with smtplib.SMTP_SSL("smtp.gmail.com", 465) as smtp:
            ...
            print(f"📧 Email alert sent for txn_id #{row['txn_id']} | step {row['step']}")
